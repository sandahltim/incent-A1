/* script.js /
/ Version: 1.2.25 /
/ Note: Added removeAttribute('readonly') and removeAttribute('disabled') to inputs in quick adjust modal to ensure editability. Force set z-index for modal to 1100 and backdrop to 1095 in handleModalShown to guarantee visibility and prevent blocking. Updated z-index logging note to reflect changes. Fixed potential overlay issues. Ensured compatibility with incentive.html (version 1.2.17), admin_manage.html (version 1.2.15), quick_adjust.html (version 1.2.7), app.py (version 1.2.32), and style.css (version 1.2.10). No changes to core functionality. */
...
${emp.employee_id} ${emp.name} ${emp.score} ${emp.role.charAt(0).toUpperCase() + emp.role.slice(1)} $${payout} `; scoreboardTable.insertAdjacentHTML('beforeend', row); }); }) .catch(error => console.error('Error updating scoreboard:', error)); }
